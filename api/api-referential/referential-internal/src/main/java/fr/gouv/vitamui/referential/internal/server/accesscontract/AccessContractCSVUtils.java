...
public class AccessContractCSVUtils extends ImportCSVUtils {
    ...
    private static List<ColumnDetails> buildAccessContractColumns(boolean isIdentifierMandatory) {
        ...
        expectedColumns.add(ColumnDetails.builder().index(4).columnName(WRITING_PERMISSION).columnType(ColumnType.A_BOOLEAN).mandatory(false).build());
        expectedColumns.add(ColumnDetails.builder().index(5).columnName(EVERY_ORIGINATING_AGENCY).columnType(ColumnType.A_BOOLEAN).mandatory(false).build());
        expectedColumns.add(ColumnDetails.builder().index(6).columnName(ORIGINATING_AGENCY).columnType(ColumnType.STRING).mandatory(false).build());
        expectedColumns.add(ColumnDetails.builder().index(7).columnName(EVERY_DATA_OBJECT_VERSION).columnType(ColumnType.A_BOOLEAN).mandatory(false).build());
        expectedColumns.add(ColumnDetails.builder().index(8).columnName(DATA_OBJECT_VERSION).columnType(ColumnType.DATA_OBJECT_VERSION_TYPE).mandatory(false).build());
        expectedColumns.add(ColumnDetails.builder().index(9).columnName(ROOT_UNITS).columnType(ColumnType.STRING).mandatory(false).build());
        expectedColumns.add(ColumnDetails.builder().index(10).columnName(EXCLUDED_ROOT_UNITS).columnType(ColumnType.STRING).mandatory(false).build());
        expectedColumns.add(ColumnDetails.builder().index(11).columnName(ACCESS_LOG).columnType(ColumnType.CONTEXT_STATUS).mandatory(false).build());
        expectedColumns.add(ColumnDetails.builder().index(12).columnName(RULE_CATEGORY_TO_FILTER).columnType(ColumnType.RULE_TYPE).mandatory(false).build());
        expectedColumns.add(ColumnDetails.builder().index(13).columnName(WRITING_RESTRICTED_DESC).columnType(ColumnType.A_BOOLEAN).mandatory(false).build());
