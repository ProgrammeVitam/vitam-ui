package fr.gouv.vitamui.iam.external.server.owner.service;
...
public class OwnerServiceIntegrationTest extends AbstractLogbookIntegrationTest {
    ...
    private OwnerDto createOwner() {
        ...
        Mockito.when(externalSecurityService.getApplicationId()).thenReturn("appId");
    ...
    public void testFindOwnerHistory() throws VitamClientException {
        ...
        Mockito.when(externalSecurityService.getTenantIdentifier()).thenReturn(tenant.getIdentifier());
        Mockito.when(externalSecurityService.getTenant(eq(tenant.getIdentifier()))).thenReturn(tenant);
        ...
        final LogbookOperationsResponseDto historyResult = ownerService.findHistoryById(ownerCreated.getId());

        assertThat(historyResult).isNotNull();
        Mockito.verify(externalSecurityService).getTenantIdentifier();
        Mockito.verify(externalSecurityService).getTenant(tenant.getIdentifier());
    }
