...
public class ProfileServiceTest extends AbstractMongoTests {
    ...
    public void testCreateProfileUser() throws Exception {
        ...
        when(externalSecurityService.userIsRootLevel()).thenCallRealMethod();
        when(externalSecurityService.getUser()).thenReturn(user);
        when(externalSecurityService.getLevel()).thenReturn("");
        when(externalSecurityService.isLevelAllowed(ArgumentMatchers.any())).thenReturn(true);
