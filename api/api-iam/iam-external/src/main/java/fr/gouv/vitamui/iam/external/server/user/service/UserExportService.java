...
public class UserExportService {
    ...
    private static final List<String> USER_LIST_COLUMNS_TITLES = List.of(
        "Identifiant du user",
        "Nom",
        "Prénom",
        "Adresse e-mail",
        ...
        "Niveau du groupe ",
        "Groupe de profils",
        "Type (nominatif ou générique)",
        "Compte subrogeable par le support (O/N)",
        "Validation en deux étapes (O/N)",
        "Mise à jour automatique SSO",
        "Date de dernière connexion",
        "Compte (actif / inactif)"
    );

    private static final List<String> USER_OPERATIONS_COLUMNS_TITLES = List.of(
        "Identifiant du user",
        "Date d’activité",
        "Heure d’activité",
        "Activité réalisée par le user",
        "Activité",
        "Ancienne valeur",
        "Nouvelle valeur"
    );
    ...
    public void createXlsxFile(
        List<UserDto> data,
        List<LogbookEventDto> userOperations,
        Map<String, String> userInfoLangMap,
        Map<String, String> userGroupNameMap,
        OutputStream xlsOutputStream
    ) throws IOException {
    ...
    private void buildUserListSheet(
        Workbook workbook,
        Sheet sheet,
        List<UserDto> users,
        Map<String, String> userInfoLangMap,
    ...
    protected void insertUserRows(
        ...
        for (UserDto user : users) {
            var row = sheet.createRow(rowNum++);
            var lastConnectionDate = user.getLastConnection() == null
                ? null
                : user.getLastConnection().format(DATE_FORMAT);
            ...
            var centerCodes = user.getCenterCodes() != null && !user.getCenterCodes().isEmpty()
                ? String.join(",", user.getCenterCodes())
                : "";
