package fr.gouv.vitamui.iam.external.server.group.dto;
...
public class LinkedProfileExport implements LineDto {
    ...
    public static LinkedProfileExport of(GroupDto groupDto) {
        return LinkedProfileExport.builder().groupIdentifier(parseDouble(groupDto.getIdentifier())).build();
    }
    ...
    public static Stream<LinkedProfileExport> createListLinked(List<GroupDto> groupsDto) {
        return groupsDto
            ...
            .flatMap(group -> {
                ...
                return group.getProfiles().stream().map(profile -> LinkedProfileExport.of(group, profile));
            });
    }
    ...
    public Map<String, ValueDto> toSheetLine() {
        return Map.of(
            LINKED_PROFILE_SHEET_COLUMNS.get(0),
            ValueDto.of(groupIdentifier, DOUBLE),
            LINKED_PROFILE_SHEET_COLUMNS.get(1),
            ValueDto.of(applicationName, STRING),
            LINKED_PROFILE_SHEET_COLUMNS.get(2),
            ValueDto.of(tenantIdentifier, DOUBLE),
            LINKED_PROFILE_SHEET_COLUMNS.get(3),
            ValueDto.of(tenantName, STRING),
            LINKED_PROFILE_SHEET_COLUMNS.get(4),
            ValueDto.of(profileIdentifier, STRING),
            LINKED_PROFILE_SHEET_COLUMNS.get(5),
            ValueDto.of(profileName, STRING)
        );
    }
}
