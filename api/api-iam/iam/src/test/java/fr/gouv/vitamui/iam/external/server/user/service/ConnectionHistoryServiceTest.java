...
import fr.gouv.vitamui.iam.external.server.user.converter.ConnectionHistoryConverter;
import fr.gouv.vitamui.iam.external.server.user.dao.ConnectionHistoryRepository;
import fr.gouv.vitamui.iam.external.server.user.domain.ConnectionHistory;
...
class ConnectionHistoryServiceTest {
    ...
    void exportConnectionHistory() throws IOException {
        // Given
        String criteria =
            "{\"criteria\":[{\"key\":\"reportingDate\",\"operator\":\"BETWEEN\",\"value\":{\"start\":\"2023-01-01T00:00:00.000Z\",\"end\":\"2023-11-21T16:57:52.099Z\"}}]}";
        ...
        when(userService.findByCustomerId("customerId")).thenReturn(List.of(user));

        when(
            connectionHistoryRepository.findAllByConnectionDateTimeBetweenAndUserIdIn(
                Date.from(Instant.parse("2023-01-01T00:00:00.000Z")),
                Date.from(Instant.parse("2023-11-21T16:57:52.099Z")),
                List.of("userId")
            )
        ).thenReturn(List.of(createConnectionHistory("userId", "2023-02-01T10:30:00.330Z", "")));
        ...
        assertThat(sheet0.getRow(1).getCell(1).getLocalDateTimeCellValue().toLocalDate()).isEqualTo(
