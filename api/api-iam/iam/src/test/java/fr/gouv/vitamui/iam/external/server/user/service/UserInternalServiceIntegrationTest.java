...
public final class UserInternalServiceIntegrationTest extends AbstractLogbookIntegrationTest {
    ...
    public void setUp() throws NoSuchFieldException, SecurityException, Exception {
        ...
        externalHttpContext = mock(ExternalHttpContext.class);
        ...
        when(externalSecurityService.hasRole(eq(ServicesData.ROLE_GET_USERS_ALL_CUSTOMERS))).thenReturn(true);
        when(externalSecurityService.hasRole(eq(ServicesData.ROLE_GENERIC_USERS))).thenReturn(true);
    ...
    public void testGetUserProfileByTokenNoTokenInDatabase() {
        when(externalHttpContext.getUserToken()).thenReturn(TOKEN_VALUE);
        Assertions.assertThrows(
            BadCredentialsException.class,
            () -> iamAuthentificationService.getUserFromHttpContext(externalHttpContext)
        );
    }
    ...
    public void testGetUserProfileByToken() {
        ...
        when(externalSecurityService.getLevel()).thenReturn(LEVEL);
        ...
        when(externalHttpContext.getUserToken()).thenReturn(TOKEN_VALUE);
        final UserDto userProfile = iamAuthentificationService.getUserFromHttpContext(externalHttpContext);
    ...
    public void testCreateAnotherUserAdmin() {
        ...
        Mockito.when(externalSecurityService.isLevelAllowed(any())).thenReturn(true);
    ...
    public void testPatch() {
        final UserDto user = createUser();
        Mockito.when(externalSecurityService.getCustomerId()).thenReturn(user.getCustomerId());
    ...
    public void testCheckExist() {
        ...
        Mockito.when(externalSecurityService.getUser()).thenReturn(userDto);
    ...
    public void testCheckExistAdminUser() {
        ...
        Mockito.when(externalSecurityService.getUser()).thenReturn(mainUserDto);
