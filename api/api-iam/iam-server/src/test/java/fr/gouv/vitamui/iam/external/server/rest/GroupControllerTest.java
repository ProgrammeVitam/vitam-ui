 ...
 *
 */
public final class GroupControllerTest implements ExternalCrudControllerTest {
    ...
    public void setup() {
        ...
        controller = new GroupController(groupService, externalSecurityService);
    }
    ...
    protected void prepareServices() {
        ...
        when(externalSecurityService.isLevelAllowed(dto.getLevel())).thenCallRealMethod();
        when(externalSecurityService.getLevel()).thenReturn("");
        when(externalSecurityService.getCustomerId()).thenReturn(dto.getCustomerId());
