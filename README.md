...
* Si une version d'ansible à été installée via `apt-get install`, il est nécessaire de la
...
- Copy files: `collect-external-client.conf`,`access-external-client.conf`, `ingest-external-client.conf`
  , `keystore_ihm-demo.p12` and `truststore_ihm-demo.jks`
  into `api/api-(iam|referential)/(iam|referential)-internal/src/main/config/dev-vitam/`
...
In order to build and package UI projects (i.e. Java backend & Angular frontend altogether), we use the
plugin `frontend-maven-plugin` provided by `com.github.eirslett`.
