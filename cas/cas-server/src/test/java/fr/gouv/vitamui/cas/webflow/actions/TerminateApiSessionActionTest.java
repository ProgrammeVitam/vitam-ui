...
public final class TerminateApiSessionActionTest extends BaseWebflowActionTest {
    ...
    private TicketGrantingTicket tgtCarrying(
        ...
        when(
            f.utils.getAttributeValue(any(), org.mockito.ArgumentMatchers.eq(CommonConstants.AUTHTOKEN_ATTRIBUTE))
        ).thenReturn(authToken);
        when(
            f.utils.getAttributeValue(any(), org.mockito.ArgumentMatchers.eq(CommonConstants.SUPER_USER_ATTRIBUTE))
        ).thenReturn(superUser);
        when(
            f.utils.getAttributeValue(
                any(),
                org.mockito.ArgumentMatchers.eq(CommonConstants.SUPER_USER_CUSTOMER_ID_ATTRIBUTE)
            )
