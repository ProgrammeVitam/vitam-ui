...
public class CertificateParserTest {
    ...
    private static final String ISSUER_DN =
        "EMAILADDRESS=admin@email.fr, CN=admin, OU=IT, O=AdminCompany, L=Paris, ST=Some-State, C=FR";
    private static final String SUBJECT_DN =
        "UID=348606, OID.2.16.840.1.113730.3.1.3=246635, C=FR, ST=Some-State, L=Paris, OU=IT, O=BobCompany, CN=bob, EMAILADDRESS=bob@email.fr";
    ...
    public void testIssuerDnParsingNoExpansion() throws CertificateParsingException {
        assertEquals(
            ...
            CertificateParser.extract(
                cert,
                new X509AttributeMapping(X509CertificateAttributes.ISSUER_DN.toString(), ".*O=(.*), L=.*", null)
            )
        );
    }
    ...
    public void testIssuerDnParsingExpansion() throws CertificateParsingException {
        assertEquals(
            "AdminCompany@email.fr",
            CertificateParser.extract(
                cert,
                new X509AttributeMapping(
                    X509CertificateAttributes.ISSUER_DN.toString(),
                    ".*O=(.*), L=.*",
                    "{0}@email.fr"
                )
            )
    ...
    public void testSubjectAlternateNameNoParsingExpansion() throws CertificateParsingException {
        final String extractSubjAltName = CertificateParser.extract(
            cert,
    ...
    public void testSubjectAlternateNameParsingNoExpansion() throws CertificateParsingException {
        final String extractedMail = CertificateParser.extract(
            cert,
            new X509AttributeMapping(
                X509CertificateAttributes.SUBJECT_ALTERNATE_NAME.toString(),
                ".*RFC822Name=(.*)",
                null
            )
        );
