...
public final class IamSurrogateAuthenticationServiceTest {
    ...
    public void testCanAuthenticateOk() {
        ...
        when(casApi.validateSubrogation(any())).thenReturn(new SubrogationValidateResponseDto(SU_ID, SURROGATE));

        assertTrue(service.canImpersonateInternal(SURROGATE, principal(), Optional.empty()));
    }

    @Test
    public void testCanAuthenticateRefusedWhenTheIamRejectsIt() {
        givenSubrogationInRequestContext();

        when(casApi.validateSubrogation(any())).thenThrow(new NotFoundException("no accepted subrogation"));
