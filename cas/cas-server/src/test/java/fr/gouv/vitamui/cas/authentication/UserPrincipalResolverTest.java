...
public final class UserPrincipalResolverTest extends BaseWebflowActionTest {
    ...
    public void testResolveX509() throws Throwable {
        // The single-CERTIFICAT-provider resolution now lives in the IAM; the resolver just consumes its
        // answer (provider id + customer id).
    ...
    public void testResolveX509NoSingleCertificateProviderReturnsNullPrincipal() throws Throwable {
        // When the IAM refuses the resolution (no CERTIFICAT provider, or several - multi-domain), the
        // resolver must stop with a NullPrincipal, exactly as before the rule moved to the IAM.
    ...
    public void testResolveAuthnDelegationMailAttributeNoValue() throws Throwable {
        ...
        // The mapped attribute is empty. CAS no longer decides on it: it forwards the raw profile and the
        // IAM refuses the login (the mapping and its checks moved there - see CasServiceDelegatedIdentityTest
        // .refusesWhenAMappedAttributeIsMissing). A refusal maps to a null principal.
    ...
    public void testResolveAuthnDelegationIdentifierAttributeNoValue() throws Throwable {
        ...
        // The mapped identifier attribute is empty: CAS forwards the raw profile and the IAM refuses the
        // login (the mapping moved there). A refusal maps to a null principal.
    ...
    public void testResolveAuthnDelegationSurrogateMailAttributeNoMail() throws Throwable {
        ...
        // No mail attribute on the surrogate profile: CAS forwards the raw profile and the IAM refuses the
        // subrogation (the mapping and the e-mail check moved there). A refusal maps to a null principal.
        doThrow(new BadRequestException("no mapped mail attribute")).when(casApi).buildPrincipalAttributes(any());
