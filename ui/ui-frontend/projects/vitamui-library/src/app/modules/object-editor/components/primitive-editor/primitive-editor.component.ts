...
export class PrimitiveEditorComponent implements OnInit {
  ...
  getPickerType(editObject: EditObject) {
    if (editObject.pattern) {
      if (editObject.pattern === DatePatternConstants.YEAR.toString()) {
        return 'year';
      } else if (editObject.pattern === DatePatternConstants.YEAR_MONTH_DAY.toString()) {
        return 'day';
      } else if (editObject.pattern === DatePatternConstants.YEAR_MONTH.toString()) {
        return 'month';
      }
    }
    return 'day';
