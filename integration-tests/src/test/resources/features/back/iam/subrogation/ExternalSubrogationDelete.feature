@Api
@ApiIam
@ApiIamSubrogations
@ApiIamSubrogationsDelete
Feature: API subrogations : suppression

  Scenario: D'une subrogation
    Given une subrogation a été créée
    When un utilisateur avec le rôle ROLE_DELETE_SUBROGATIONS supprime une subrogation dans un tenant auquel il est autorisé en utilisant un certificat full access avec le rôle ROLE_DELETE_SUBROGATIONS
    Then le serveur ne retourne plus la subrogaton supprimée

  Scenario Outline: D'une subrogation, cas sécurité, par un utilisateur <(userRole) avec ou sans> le rôle ROLE_DELETE_SUBROGATIONS sur le tenant principal, après avoir choisi le tenant <(headerTenant) principal ou secondaire> dans l'IHM, et en utilisant un certificat <(certRole) avec ou sans> le rôle ROLE_DELETE_SUBROGATIONS <(certTenant) sur le tenant principal ou sur le tenant secondaire ou étant fullAccess> = le serveur <autorise ou refuse> l'accès à l'api subrogations
    Given on demande à ce que le subrogateur soit le user de test
    And une subrogation a été créée
    And deux tenants et un rôle par défaut pour la suppression d'une subrogation
    And un utilisateur <(userRole) avec ou sans> le rôle ROLE_DELETE_SUBROGATIONS sur le tenant principal
    And l'utilisateur a selectionné le tenant <(headerTenant) principal ou secondaire> dans l'IHM
    And un certificat <(certRole) avec ou sans> le rôle ROLE_DELETE_SUBROGATIONS <(certTenant) sur le tenant principal ou sur le tenant secondaire ou étant fullAccess>
    When cet utilisateur supprime une subrogation
    Then le serveur <autorise ou refuse> l'accès à l'API subrogations

  Examples:
    | (userRole) avec ou sans | (headerTenant) principal ou secondaire | (certRole) avec ou sans | (certTenant) sur le tenant principal ou sur le tenant secondaire ou étant fullAccess | autorise ou refuse |
    | avec | principal | avec | sur le tenant principal | autorise |
    | sans | principal | avec | sur le tenant principal | refuse |
    | avec | secondaire | avec | sur le tenant principal | refuse |
    | sans | secondaire | avec | sur le tenant principal | refuse |
    | avec | principal | sans | sur le tenant principal | refuse |
    | sans | principal | sans | sur le tenant principal | refuse |
    | avec | secondaire | sans | sur le tenant principal | refuse |
    | sans | secondaire | sans | sur le tenant principal | refuse |
    | avec | principal | avec | sur le tenant secondaire | refuse |
    | sans | principal | avec | sur le tenant secondaire | refuse |
    | avec | secondaire | avec | sur le tenant secondaire | refuse |
    | sans | secondaire | avec | sur le tenant secondaire | refuse |
    | avec | principal | sans | sur le tenant secondaire | refuse |
    | sans | principal | sans | sur le tenant secondaire | refuse |
    | avec | secondaire | sans | sur le tenant secondaire | refuse |
    | sans | secondaire | sans | sur le tenant secondaire | refuse |
    | avec | principal | avec | étant fullAccess | autorise |
    | sans | principal | avec | étant fullAccess | refuse |
    | avec | secondaire | avec | étant fullAccess | refuse |
    | sans | secondaire | avec | étant fullAccess | refuse |
    | avec | principal | sans | étant fullAccess | refuse |
    | sans | principal | sans | étant fullAccess | refuse |
    | avec | secondaire | sans | étant fullAccess | refuse |
    | sans | secondaire | sans | étant fullAccess | refuse |

  Scenario: D'une subrogation par un utilisateur qui n'est pas le subrogateur
    Given une subrogation a été créée avec un autre subrogateur
    When un utilisateur avec le rôle ROLE_DELETE_SUBROGATIONS supprime une subrogation dont il n'est pas le subrogateur dans un tenant auquel il est autorisé en utilisant un certificat full access avec le rôle ROLE_DELETE_SUBROGATIONS
    Then le serveur refuse la suppression de la subrogation à cause du mauvais subrogateur
