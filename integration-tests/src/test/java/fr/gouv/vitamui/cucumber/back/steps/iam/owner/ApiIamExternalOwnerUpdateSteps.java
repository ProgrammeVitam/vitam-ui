...
public class ApiIamExternalOwnerUpdateSteps extends CommonSteps {
    ...
    public void un_propriétaire_a_été_créé() {
        ...
        testContext.savedOwnerDto = getOwnerRestClient()
            .create(getSystemTenantUserAdminContext(), testContext.ownerDto);
    }
    ...
    public void le_serveur_retourne_le_propriétaire_mis_à_jour() {
        assertThat(testContext.ownerDto.getName()).isEqualTo(
            TestConstants.UPDATED + testContext.savedOwnerDto.getName()
        );
    }

    @When(
        "^un utilisateur avec le rôle ROLE_UPDATE_OWNERS met à jour un propriétaire dans un tenant auquel il est autorisé en utilisant un certificat full access avec le rôle ROLE_UPDATE_OWNERS$"
    )
    ...
    @When(
        "^un utilisateur avec le rôle ROLE_UPDATE_OWNERS met à jour un propriétaire en readonly dans un tenant auquel il est autorisé en utilisant un certificat full access avec le rôle ROLE_UPDATE_OWNERS$"
    )
    ...
    public void le_serveur_refuse_la_mise_à_jour_du_propriétaire() {
        ...
        assertThat(testContext.exception.toString()).isEqualTo(
            "fr.gouv.vitamui.commons.api.exception.InvalidFormatException: The owner " +
            testContext.ownerDto.getName() +
            " can't be updated."
        );
    }
