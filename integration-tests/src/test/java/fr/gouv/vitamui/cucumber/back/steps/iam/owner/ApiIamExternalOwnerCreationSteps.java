...
public class ApiIamExternalOwnerCreationSteps extends CommonSteps {
    ...
    @When(
        "^un utilisateur avec le rôle ROLE_CREATE_OWNERS ajoute un nouveau propriétaire dans un tenant auquel il est autorisé en utilisant un certificat full access avec le rôle ROLE_CREATE_OWNERS$"
    )
    public void un_utilisateur_avec_le_rôle_ROLE_CREATE_OWNERS_ajoute_un_nouveau_propriétaire_dans_un_tenant_auquel_il_est_autorisé_en_utilisant_un_certificat_full_access_avec_le_rôle_ROLE_CREATE_OWNERS() {
        testContext.ownerDto = getOwnerRestClient()
            .create(getSystemTenantUserAdminContext(), FactoryDto.buildDto(OwnerDto.class));
    }
