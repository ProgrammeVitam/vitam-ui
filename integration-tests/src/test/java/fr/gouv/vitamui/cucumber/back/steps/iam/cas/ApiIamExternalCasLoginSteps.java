...
public class ApiIamExternalCasLoginSteps extends CommonSteps {
    ...
    @When(
        "^un utilisateur avec le rôle ROLE_CAS_LOGIN authentifie un utilisateur dans un tenant auquel il est autorisé en utilisant un certificat sur le tenant et avec le rôle ROLE_CAS_LOGIN$"
    )
    public void un_utilisateur_avec_le_rôle_ROLE_CAS_LOGIN_authentifie_un_utilisateur_dans_un_tenant_auquel_il_est_autorisé_en_utilisant_un_certificat_sur_le_tenant_et_avec_le_rôle_ROLE_CAS_LOGIN() {
        testContext.authUserDto = new AuthUserDto(
            getCasRestClient(
                ...
                new String[] { ServicesData.ROLE_CAS_LOGIN }
            ).login(
    ...
    @When(
        "^un utilisateur avec le rôle ROLE_CAS_LOGIN authentifie un utilisateur avec un mauvais mot de passe dans un tenant auquel il est autorisé en utilisant un certificat sur le tenant et avec le rôle ROLE_CAS_LOGIN$"
    )
    ...
    private void createUserAndSetPassword() {
        ...
        getCasRestClient(
            false,
            new Integer[] { casTenantIdentifier },
            new String[] { ServicesData.ROLE_CAS_CHANGE_PASSWORD }
        ).changePassword(
            getContext(casTenantIdentifier, TestConstants.TOKEN_USER_CAS),
            testContext.authUserDto.getEmail(),
            testContext.authUserDto.getCustomerId(),
    ...
    public void le_serveur_retourne_une_erreur_bad_credentials() {
        ...
        assertThat(testContext.exception.toString()).isEqualTo(
            "fr.gouv.vitamui.commons.api.exception.InvalidAuthenticationException: Bad credentials for username: " +
            testContext.authUserDto.getEmail()
        );
    }

    @When(
        "^un utilisateur avec le rôle ROLE_CAS_LOGIN authentifie un utilisateur générique dans un tenant auquel il est autorisé en utilisant un certificat sur le tenant et avec le rôle ROLE_CAS_LOGIN$"
    )
    ...
    @When(
        "^un utilisateur avec le rôle ROLE_CAS_LOGIN authentifie un utilisateur avec un mauvais mot de passe trop de fois dans un tenant auquel il est autorisé en utilisant un certificat sur le tenant et avec le rôle ROLE_CAS_LOGIN$"
    )
        ...
        for (int i = 0; i < 4; i++) {
            try {
                getCasRestClient(
                    ...
                    new Integer[] { casTenantIdentifier },
                    new String[] { ServicesData.ROLE_CAS_LOGIN }
                ).login(context, login, customerId, BAD_PASSWORD, null, null, null);
            } catch (final RuntimeException e) {
    ...
    public void le_serveur_retourne_une_erreur_de_trop_d_essais() {
        assertThat(testContext.exception).isNotNull();
        assertThat(testContext.exception.toString()).isEqualTo(
            "fr.gouv.vitamui.commons.api.exception.TooManyRequestsException: Too many login attempts for username: " +
            testContext.authUserDto.getEmail()
        );
    }
    ...
    public void l_utilisateur_est_bloqué() {
        final ExternalHttpContext context = getContext(casTenantIdentifier, TestConstants.TOKEN_USER_CAS);
        final List<UserDto> basicUsersDto = getCasRestClient(
            false,
            new Integer[] { casTenantIdentifier },
            new String[] { ServicesData.ROLE_CAS_USERS }
        ).getUsersByEmail(context, testContext.authUserDto.getEmail(), Optional.empty());
        assertThat(basicUsersDto).hasSize(1);
        assertThat(basicUsersDto.get(0).getStatus()).isEqualTo(UserStatusEnum.BLOCKED);
    }

    @When(
        "^un utilisateur avec le rôle ROLE_CAS_LOGIN authentifie un utilisateur désactivé dans un tenant auquel il est autorisé en utilisant un certificat sur le tenant et avec le rôle ROLE_CAS_LOGIN$"
    )
    ...
    @When(
        "^un utilisateur avec le rôle ROLE_CAS_LOGIN authentifie un utilisateur inexistant dans un tenant auquel il est autorisé en utilisant un certificat sur le tenant et avec le rôle ROLE_CAS_LOGIN$"
    )
    public void un_utilisateur_avec_le_rôle_ROLE_CAS_LOGIN_authentifie_un_utilisateur_inexistant_dans_un_tenant_auquel_il_est_autorisé_en_utilisant_un_certificat_sur_le_tenant_et_avec_le_rôle_ROLE_CAS_LOGIN() {
        try {
            getCasRestClient(
                ...
                getContext(casTenantIdentifier, TestConstants.TOKEN_USER_CAS),
                TestConstants.BAD_LOGIN,
                TestConstants.CLIENT1_CUSTOMER_ID,
                adminPassword,
                null,
                null,
                null
