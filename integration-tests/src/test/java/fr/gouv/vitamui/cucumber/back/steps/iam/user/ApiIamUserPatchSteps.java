...
public class ApiIamUserPatchSteps extends CommonSteps {
    ...
    @When(
        "^un utilisateur avec le rôle ROLE_UPDATE_USERS met à jour partiellement un utilisateur dans un tenant auquel il est autorisé en utilisant un certificat full access avec le rôle ROLE_UPDATE_USERS$"
    )
    public void un_utilisateur_avec_le_rôle_ROLE_UPDATE_USERS_met_à_jour_partiellement_un_utilisateur_dans_un_tenant_auquel_il_est_autorisé_en_utilisant_un_certificat_full_access_avec_le_rôle_ROLE_UPDATE_USERS() {
        patchedUser = getUserRestClient(true, null, new String[] { ROLE_UPDATE_USERS }).patch(
    ...
    @When(
        "^un utilisateur avec le rôle ROLE_UPDATE_USERS met à jour partiellement un utilisateur en readonly dans un tenant auquel il est autorisé en utilisant un certificat full access avec le rôle ROLE_UPDATE_USERS$"
    )
    public void un_utilisateur_avec_le_rôle_ROLE_UPDATE_USERS_met_à_jour_partiellement_un_utilisateur_en_readonly_dans_un_tenant_auquel_il_est_autorisé_en_utilisant_un_certificat_full_access_avec_le_rôle_ROLE_UPDATE_USERS() {
        ...
        try {
            getUserRestClient(true, null, new String[] { ROLE_UPDATE_USERS }).patch(
                getSystemTenantUserAdminContext(),
                patch
            );
    ...
    public void le_serveur_refuse_la_mise_à_jour_partielle_de_l_utilisateur_à_cause_du_readonly() {
        ...
        assertThat(testContext.exception.toString()).isEqualTo(
            "fr.gouv.vitamui.commons.api.exception.InvalidFormatException: Unable to patch user " +
            patchedUser.getId() +
            patchedUser.getId() +
            " cannot patch readonly"
        );
    }

    @When(
        "^un utilisateur avec le rôle ROLE_UPDATE_USERS met à jour partiellement un utilisateur mais avec un mauvais client dans un tenant auquel il est autorisé en utilisant un certificat full access avec le rôle ROLE_UPDATE_USERS$"
    )
    ...
    public void le_serveur_refuse_la_mise_à_jour_partielle_de_l_utilisateur_à_cause_du_mauvais_client() {
        assertThat(testContext.exception).isNotNull();
        assertThat(testContext.exception.toString()).isEqualTo(
            "fr.gouv.vitamui.commons.api.exception.InvalidFormatException: Unable to patch user: customerId " +
            CLIENT1_CUSTOMER_ID +
            " is not allowed"
        );
    }
    ...
    @When(
        "^un utilisateur avec le rôle ROLE_UPDATE_USERS met à jour partiellement un utilisateur mais avec un email existant dans un tenant auquel il est autorisé en utilisant un certificat full access avec le rôle ROLE_UPDATE_USERS$"
    )
    ...
    @When(
        "^un utilisateur avec le rôle ROLE_UPDATE_USERS met à jour partiellement un utilisateur mais avec un groupe inexistant dans un tenant auquel il est autorisé en utilisant un certificat full access avec le rôle ROLE_UPDATE_USERS$"
    )
    ...
    @When(
        "^un utilisateur avec le rôle ROLE_UPDATE_USERS sans le bon niveau met à jour partiellement un utilisateur dans un tenant auquel il est autorisé en utilisant un certificat full access avec le rôle ROLE_UPDATE_USERS$"
    )
    ...
    @When(
        "^un utilisateur avec le rôle ROLE_UPDATE_ME_USERS se met à jour partiellement dans un tenant auquel il est autorisé en utilisant un certificat full access avec le rôle ROLE_UPDATE_ME_USERS$"
    )
    public void un_utilisateur_avec_le_rôle_ROLE_UPDATE_ME_USERS_se_met_à_jour_partiellement_dans_un_tenant_auquel_il_est_autorisé_en_utilisant_un_certificat_full_access_avec_le_rôle_ROLE_UPDATE_ME_USERS() {
        ...
        patchedUser = getUserRestClient(true, null, new String[] { ROLE_UPDATE_ME_USERS }).patchMe(
            getContext(proofTenantIdentifier, TestConstants.TESTS_TOKEN_ID),
            buildUserToPatch()
        );
    }

    @When(
        "^un utilisateur avec le rôle ROLE_UPDATE_ME_USERS se met à jour partiellement avec un autre identifiant dans un tenant auquel il est autorisé en utilisant un certificat full access avec le rôle ROLE_UPDATE_ME_USERS$"
    )
    ...
    @When(
        "^un utilisateur avec le rôle ROLE_UPDATE_ME_USERS se met à jour partiellement dans un tenant auquel il est autorisé en utilisant un certificat full access sans le rôle ROLE_UPDATE_ME_USERS$"
    )
    public void un_utilisateur_avec_le_rôle_ROLE_UPDATE_ME_USERS_se_met_à_jour_partiellement_dans_un_tenant_auquel_il_est_autorisé_en_utilisant_un_certificat_full_access_sans_le_rôle_ROLE_UPDATE_ME_USERS() {
        writeToken(TestConstants.TESTS_TOKEN_ID, testContext.savedUserDto.getId());
        try {
            getUserRestClient(true, null, new String[] { ROLE_LOGBOOKS }).patchMe(
                getContext(proofTenantIdentifier, TestConstants.TESTS_TOKEN_ID),
                buildUserToPatch()
            );
        } catch (final RuntimeException e) {
