...
import fr.gouv.vitamui.iam.client.GroupExternalRestClient;
...
public class ApiIamExternalGroupGetSteps extends CommonSteps {
    ...
    @When(
        "^un utilisateur avec le rôle ROLE_GET_GROUPS récupère tous les groupes dans un tenant auquel il est autorisé en utilisant un certificat full access avec le rôle ROLE_GET_GROUPS$"
    )
    ...
    @When(
        "^un utilisateur avec le rôle ROLE_GET_GROUPS sans le bon niveau récupère tous les groupes dans un tenant auquel il est autorisé en utilisant un certificat full access avec le rôle ROLE_GET_GROUPS$"
    )
    ...
    private ExternalHttpContext getGoodContextWithWrongLevel() {
        ...
        return getContext(
            ...
            tokenUserTest(new String[] { ROLE_GET_GROUPS }, proofTenantIdentifier, SYSTEM_CUSTOMER_ID, WRONG_LEVEL)
        );
    }
    ...
    public void le_serveur_ne_retourne_que_le_groupe_de_l_utilisateur() throws Exception {
        ...
        assertThat(groupDtos.get(0).getLevel())
            .overridingErrorMessage(
                "Le groupe retourné n'a pas le niveau attendu : " +
                groupDtos.get(0).getLevel() +
                " au lieu de " +
    ...
    @When(
        "^un utilisateur avec le rôle ROLE_GET_GROUPS récupère un groupe par son identifiant dans un tenant auquel il est autorisé en utilisant un certificat full access avec le rôle ROLE_GET_GROUPS$"
    )
    ...
    @When(
        "^un utilisateur avec le rôle ROLE_GET_GROUPS sans le bon niveau récupère un groupe par son identifiant dans un tenant auquel il est autorisé en utilisant un certificat full access avec le rôle ROLE_GET_GROUPS$"
    )
    ...
    @When(
        "^un utilisateur avec le rôle ROLE_GET_GROUPS récupère tous les groupes avec pagination dans un tenant auquel il est autorisé en utilisant un certificat full access avec le rôle ROLE_GET_GROUPS$"
    )
    public void un_utilisateur_avec_le_rôle_ROLE_GET_GROUPS_récupère_tous_les_groupes_avec_pagination_dans_un_tenant_auquel_il_est_autorisé_en_utilisant_un_certificat_full_access_avec_le_rôle_ROLE_GET_GROUPS() {
        final PaginatedValuesDto<GroupDto> groups = getGroupRestClient()
            .getAllPaginated(
    ...
    @When(
        "^un utilisateur avec le rôle ROLE_GET_GROUPS sans le bon niveau récupère tous les groupes avec pagination dans un tenant auquel il est autorisé en utilisant un certificat full access avec le rôle ROLE_GET_GROUPS$"
    )
    public void un_utilisateur_avec_le_rôle_ROLE_GET_GROUPS_sans_le_bon_niveau_récupère_tous_les_groupes_avec_pagination_dans_un_tenant_auquel_il_est_autorisé_en_utilisant_un_certificat_full_access_avec_le_rôle_ROLE_GET_GROUPS() {
        final PaginatedValuesDto<GroupDto> groups = getGoodClient()
            .getAllPaginated(
