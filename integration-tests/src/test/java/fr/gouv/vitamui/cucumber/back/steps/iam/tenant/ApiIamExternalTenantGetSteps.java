...
public class ApiIamExternalTenantGetSteps extends CommonSteps {
    ...
    @When(
        "^un utilisateur avec le rôle ROLE_GET_ALL_TENANTS récupère un tenant par son identifiant dans un tenant auquel il est autorisé en utilisant un certificat full access avec le rôle ROLE_GET_ALL_TENANTS$"
    )
    public void un_utilisateur_avec_le_rôle_ROLE_GET_ALL_TENANTS_récupère_un_tenant_par_son_identifiant_dans_un_tenant_auquel_il_est_autorisé_en_utilisant_un_certificat_full_access_avec_le_rôle_ROLE_GET_ALL_TENANTS() {
        testContext.tenantDto = getTenantRestClient()
            .getOne(getSystemTenantUserAdminContext(), TestConstants.SYSTEM_TENANT_ID, Optional.empty());
    }
    ...
    public void cet_utilisateur_récupère_un_tenant_par_son_identifiant() {
        try {
            testContext.tenantDto = getTenantRestClient(
                ...
                TestConstants.SYSTEM_TENANT_ID,
                Optional.empty()
            );
        } catch (final RuntimeException e) {
            testContext.exception = e;
    ...
    @When(
        "^un utilisateur avec le rôle ROLE_GET_TENANTS récupère un tenant par son identifiant dans un tenant auquel il est autorisé et identique au tenant récupéré en utilisant un certificat full access avec le rôle ROLE_GET_ALL_TENANTS$"
    )
    public void un_utilisateur_avec_le_rôle_ROLE_GET_TENANTS_récupère_un_tenant_par_son_identifiant_dans_un_tenant_auquel_il_est_autorisé_et_identique_au_tenant_récupéré_en_utilisant_un_certificat_full_access_avec_le_rôle_ROLE_GET_ALL_TENANTS() {
        testContext.tenantDto = getTenantRestClient()
            .getOne(
                getContext(proofTenantIdentifier, tokenUserTestTenantSystem()),
                TestConstants.SYSTEM_TENANT_ID,
                Optional.empty()
            );
    }
    ...
    @When(
        "^un utilisateur avec le rôle ROLE_GET_TENANTS récupère un tenant par son identifiant dans un tenant auquel il est autorisé mais différent du tenant récupéré en utilisant un certificat full access avec le rôle ROLE_GET_ALL_TENANTS$"
    )
    ...
    public void l_utilisateur_n_a_pas_les_accès_suffisants_pour_récupérer_ce_tenant() {
        ...
        assertThat(testContext.exception.toString()).isEqualTo(
            "fr.gouv.vitamui.commons.api.exception.ForbiddenException: Unable to access to the tenant " +
            TestConstants.CAS_TENANT_ID +
    ...
    @When(
        "^un utilisateur avec le rôle ROLE_GET_TENANTS récupère tous les tenants par le nom dans un tenant auquel il est autorisé en utilisant un certificat full access avec le rôle ROLE_GET_TENANTS$"
    )
    public void un_utilisateur_avec_le_rôle_ROLE_GET_TENANTS_récupère_tous_les_tenants_par_le_nom_dans_un_tenant_auquel_il_est_autorisé_en_utilisant_un_certificat_full_access_avec_le_rôle_ROLE_GET_TENANTS() {
        ...
        tenantDtos = getTenantRestClient()
    ...
    @When(
        "^un utilisateur avec le rôle ROLE_GET_TENANTS récupère tous les tenants par le nom dans un tenant auquel il n'est pas autorisé en utilisant un certificat full access avec le rôle ROLE_GET_TENANTS$"
    )
    public void un_utilisateur_avec_le_rôle_ROLE_GET_TENANTS_récupère_tous_les_tenants_par_le_nom_dans_un_tenant_auquel_il_n_est_pas_autorisé_en_utilisant_un_certificat_full_access_avec_le_rôle_ROLE_GET_TENANTS() {
        try {
            final QueryDto criteria = QueryDto.criteria(
                "name",
                TestConstants.CAS_TENANT_NAME,
                CriterionOperator.EQUALS
            );
            tenantDtos = getTenantRestClient()
                .getAll(
                    getContext(casTenantIdentifier, TestConstants.TOKEN_USER_ADMIN),
