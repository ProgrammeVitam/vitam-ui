...
public class ApiIamUserCheckSteps extends CommonSteps {
    ...
    @When(
        "^un utilisateur avec le rôle ROLE_GET_USERS vérifie l'existence d'un utilisateur par son email dans un tenant auquel il est autorisé en utilisant un certificat full access avec le rôle ROLE_GET_USERS$"
    )
    ...
    @When(
        "^un utilisateur avec le rôle ROLE_GET_USERS sans le bon niveau vérifie l'existence d'un utilisateur par son email dans un tenant auquel il est autorisé en utilisant un certificat full access avec le rôle ROLE_GET_USERS$"
    )
    public void un_utilisateur_avec_le_rôle_ROLE_GET_USERS_sans_le_bon_niveau_vérifie_l_existence_d_un_utilisateur_par_son_email_dans_un_tenant_auquel_il_est_autorisé_en_utilisant_un_certificat_full_access_avec_le_rôle_ROLE_GET_USERS() {
        ...
        testContext.bResponse = getUserRestClient(
            true,
