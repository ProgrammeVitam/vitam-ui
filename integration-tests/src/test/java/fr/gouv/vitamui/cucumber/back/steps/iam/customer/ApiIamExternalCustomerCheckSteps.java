...
public class ApiIamExternalCustomerCheckSteps extends CommonSteps {
    ...
    public void un_utilisateur_vérifie_l_existence_d_un_client_par_son_identifiant() {
        try {
            ...
            testContext.bResponse = getCustomerRestClient()
    ...
    @When(
        "^un utilisateur avec le rôle ROLE_GET_CUSTOMERS vérifie l'existence d'un client par son code et domain dans un tenant auquel il est autorisé en utilisant un certificat full access avec le rôle ROLE_GET_CUSTOMERS$"
    )
    public void un_utilisateur_avec_le_rôle_ROLE_GET_CUSTOMERS_vérifie_l_existence_d_un_client_par_son_code_et_domain_dans_un_tenant_auquel_il_est_autorisé_en_utilisant_un_certificat_full_access_avec_le_rôle_ROLE_GET_CUSTOMERS() {
        final QueryDto criteria = QueryDto.criteria(
            "emailDomains",
            Arrays.asList(defaultEmailDomain),
            CriterionOperator.IN
        ).addCriterion("code", TestConstants.SYSTEM_CUSTOMER_CODE, CriterionOperator.EQUALS);
    ...
    @Given(
        "^deux tenants et un rôle par défaut pour la vérification de l'existence d'un client par son code et domain$"
    )
