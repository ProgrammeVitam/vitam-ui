...
public class ApiIamExternalGroupCreationSteps extends CommonSteps {
    ...
    @When(
        "^un utilisateur avec le rôle ROLE_CREATE_GROUPS ajoute un nouveau groupe dans un tenant auquel il est autorisé en utilisant un certificat full access avec le rôle ROLE_CREATE_GROUPS$"
    )
    ...
    @When(
        "^un utilisateur avec le rôle ROLE_CREATE_GROUPS ajoute un nouveau groupe en readonly dans un tenant auquel il est autorisé en utilisant un certificat full access avec le rôle ROLE_CREATE_GROUPS$"
    )
    ...
    public void le_serveur_refuse_la_création_du_groupe_à_cause_du_readonly() {
        ...
        assertThat(testContext.exception.toString()).isEqualTo(
            "fr.gouv.vitamui.commons.api.exception.InvalidFormatException: Unable to create group " +
            savedGroupDto.getName() +
            ": readonly must be set to false"
        );
    }

    @When(
        "^un utilisateur avec le rôle ROLE_CREATE_GROUPS mais avec un mauvais client ajoute un nouveau groupe dans un tenant auquel il est autorisé en utilisant un certificat full access avec le rôle ROLE_CREATE_GROUPS$"
    )
    ...
    public void le_serveur_refuse_la_création_du_groupe_à_cause_du_mauvais_client() {
        ...
        assertThat(testContext.exception.toString()).isEqualTo(
            "fr.gouv.vitamui.commons.api.exception.InvalidFormatException: Unable to create group " +
            savedGroupDto.getName() +
            ": profile and group customerId must be equals"
        );
    }

    @When(
        "^un utilisateur avec le rôle ROLE_CREATE_GROUPS et un client désactivé ajoute un nouveau groupe dans un tenant auquel il est autorisé en utilisant un certificat full access avec le rôle ROLE_CREATE_GROUPS$"
    )
    public void un_utilisateur_avec_le_rôle_ROLE_CREATE_GROUPS_et_un_client_désactivé_ajoute_un_nouveau_groupe_dans_un_tenant_auquel_il_est_autorisé_en_utilisant_un_certificat_full_access_avec_le_rôle_ROLE_CREATE_GROUPS() {
        ...
        final TenantDto tenant = getTenantRestClient()
        ...
        try {
            groupDto = getGroupRestClient(true, null, new String[] { ROLE_CREATE_GROUPS }).create(
                getContext(
                    tenantIdentifier,
                    tokenUserTest(new String[] { ROLE_CREATE_GROUPS }, tenantIdentifier, customerId, testContext.level)
                ),
                savedGroupDto
            );
        } catch (final RuntimeException e) {
    ...
    public void le_serveur_refuse_la_création_du_groupe_à_cause_du_client_désactivé() {
        ...
        assertThat(testContext.exception.toString()).isEqualTo(
            "fr.gouv.vitamui.commons.api.exception.InvalidFormatException: Unable to create group " +
            savedGroupDto.getName() +
            ": customer must be enabled"
        );
    }

    @When(
        "^un utilisateur avec le rôle ROLE_CREATE_GROUPS ajoute un nouveau groupe avec un nom existant dans un tenant auquel il est autorisé en utilisant un certificat full access avec le rôle ROLE_CREATE_GROUPS$"
    )
    ...
    @When(
        "^un utilisateur avec le rôle ROLE_CREATE_GROUPS ajoute un nouveau groupe avec un profil inexistant dans un tenant auquel il est autorisé en utilisant un certificat full access avec le rôle ROLE_CREATE_GROUPS$"
    )
    ...
    @When(
        "^un utilisateur avec le rôle ROLE_CREATE_GROUPS ajoute un nouveau groupe avec un profil d'un autre client dans un tenant auquel il est autorisé en utilisant un certificat full access avec le rôle ROLE_CREATE_GROUPS$"
    )
    ...
    public void cet_utilisateur_crée_un_nouveau_groupe_de_niveau_x(final LevelParameterType level) throws Exception {
        ...
        try {
            testContext.groupDto = getGroupRestClient()
                .create(getContext(testContext.mainTenant, testContext.tokenUser), group);
        } catch (final RuntimeException e) {
            testContext.exception = e;
        }
    }
    ...
    public void le_niveau_du_nouveau_groupe_est_bien_le_niveau(final LevelParameterType level) throws Exception {
        assertThat(testContext.groupDto.getLevel())
