...
public class ApiIamUserCreationSteps extends CommonSteps {
    ...
    @When(
        "^un utilisateur avec le rôle ROLE_CREATE_USERS ajoute un nouvel utilisateur dans un tenant auquel il est autorisé en utilisant un certificat full access avec le rôle ROLE_CREATE_USERS$"
    )
    public void un_utilisateur_avec_le_rôle_ROLE_CREATE_USERS_ajoute_un_nouvel_utilisateur_dans_un_tenant_auquel_il_est_autorisé_en_utilisant_un_certificat_full_access_avec_le_rôle_ROLE_CREATE_USERS() {
        newUser = getUserRestClient(true, null, new String[] { ROLE_CREATE_USERS }).create(
    ...
    @When(
        "^un utilisateur avec le rôle ROLE_CREATE_USERS ajoute un nouvel utilisateur en readonly dans un tenant auquel il est autorisé en utilisant un certificat full access avec le rôle ROLE_CREATE_USERS$"
    )
    public void un_utilisateur_avec_le_rôle_ROLE_CREATE_USERS_ajoute_un_nouvel_utilisateur_en_readonly_dans_un_tenant_auquel_il_est_autorisé_en_utilisant_un_certificat_full_access_avec_le_rôle_ROLE_CREATE_USERS() {
        ...
        try {
            getUserRestClient(true, null, new String[] { ROLE_CREATE_USERS }).create(
                getSystemTenantUserAdminContext(),
                newUser
            );
    ...
    public void le_serveur_refuse_la_création_de_l_utilisateur_à_cause_du_readonly() {
        ...
        assertThat(testContext.exception.toString()).isEqualTo(
            "fr.gouv.vitamui.commons.api.exception.InvalidFormatException: Unable to create user " +
            newUser.getEmail() +
    ...
    @When(
        "^un utilisateur avec le rôle ROLE_CREATE_USERS ajoute un nouvel utilisateur mais avec un mauvais client dans un tenant auquel il est autorisé en utilisant un certificat full access avec le rôle ROLE_CREATE_USERS$"
    )
    ...
    public void le_serveur_refuse_la_création_de_l_utilisateur_à_cause_du_mauvais_client() {
        assertThat(testContext.exception).isNotNull();
        assertThat(testContext.exception.toString()).isEqualTo(
            "fr.gouv.vitamui.commons.api.exception.InvalidFormatException: Unable to create user: customerId " +
            CLIENT1_CUSTOMER_ID +
            " is not allowed"
        );
    }
    ...
    @When(
        "^un utilisateur avec le rôle ROLE_CREATE_USERS ajoute un nouvel utilisateur mais avec un email existant dans un tenant auquel il est autorisé en utilisant un certificat full access avec le rôle ROLE_CREATE_USERS$"
    )
    ...
    public void le_serveur_refuse_la_création_de_l_utilisateur_à_cause_de_l_email_existant() {
        ...
        assertThat(testContext.exception.toString()).isEqualTo(
            "fr.gouv.vitamui.commons.api.exception.InvalidFormatException: Unable to create user " +
            TestConstants.SYSTEM_USER_PREFIX_EMAIL +
    ...
    @When(
        "^un utilisateur avec le rôle ROLE_CREATE_USERS ajoute un nouvel utilisateur mais avec un groupe inexistant dans un tenant auquel il est autorisé en utilisant un certificat full access avec le rôle ROLE_CREATE_USERS$"
    )
    ...
    public void cet_utilisateur_ajoute_un_nouvel_utilisateur() {
        ...
        dto.setEmail(
            dto
                ...
                .replaceFirst(
                    CommonConstants.EMAIL_SEPARATOR + "(.*)",
                    CommonConstants.EMAIL_SEPARATOR + defaultEmailDomain
                )
        );
        try {
            getUserRestClient(
    ...
    public void un_utilisateur_de_ce_niveau(final RoleParameterType role) {
        testContext.tokenUser = tokenUserTest(
            new String[] { role.getData() },
    ...
    public void le_nouvel_utilisateur_est_bien_affecté_au_groupe_donné() {
        ...
        assertThat(newUser.getGroupId())
            .overridingErrorMessage("Le groupe de l'utilisateur créé est vide")
            .isNotEmpty();
        assertThat(newUser.getGroupId())
            .overridingErrorMessage("Le groupe de l'utilisateur créé est incorrect")
            .isEqualTo(testContext.groupDto.getId());
    }
    ...
    public void le_serveur_refuse_l_acces_à_l_api_users() {
        assertThat(testContext.exception).isNotNull();
        assertThat(testContext.exception.toString()).contains(
            "fr.gouv.vitamui.commons.api.exception.InvalidFormatException: Unable to create user"
        );
