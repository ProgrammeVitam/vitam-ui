...
public class ApiIamExternalSubrogationGetSteps extends CommonSteps {
    ...
    @When(
        "^un utilisateur avec le rôle ROLE_GET_SUBROGATIONS récupère une subrogation par son identifiant dans un tenant auquel il est autorisé en utilisant un certificat full access avec le rôle ROLE_GET_SUBROGATIONS$"
    )
    public void un_utilisateur_avec_le_rôle_ROLE_GET_SUBROGATIONS_récupère_une_subrogation_par_son_identifiant_dans_un_tenant_auquel_il_est_autorisé_en_utilisant_un_certificat_full_access_avec_le_rôle_ROLE_GET_SUBROGATIONS() {
        subrogationDto = getSubrogationRestClient()
            .getOne(getSystemTenantUserAdminContext(), getOrInitializeDefaultSubrogationId(), Optional.empty());
    }
    ...
    public void une_subrogation_existe_pour_moi_en_tant_que_subrogé() {
        ...
        testContext.savedSubrogationDto = getSubrogationRestClient()
            .create(getSystemTenantUserAdminContext(), subrogationDto);
    ...
    public void une_subrogation_existe_pour_moi_en_tant_que_subrogateur() {
        ...
        testContext.authUserDto = (AuthUserDto) getCasRestClient(
            ...
            getContext(casTenantIdentifier, TestConstants.TOKEN_USER_CAS),
            subrogationDto.getSuperUser(),
            subrogationDto.getSuperUserCustomerId(),
            Optional.of(CommonConstants.AUTH_TOKEN_PARAMETER)
        );
    }
