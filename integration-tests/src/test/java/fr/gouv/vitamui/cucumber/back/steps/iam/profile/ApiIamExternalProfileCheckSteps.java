...
public class ApiIamExternalProfileCheckSteps extends CommonSteps {
    ...
    public void un_utilisateur_vérifie_l_existence_d_un_profil_par_son_identifiant() {
        try {
            ...
            testContext.bResponse = getProfileRestClient()
    ...
    @When(
        "^un utilisateur avec le rôle ROLE_GET_PROFILES vérifie l'existence d'un profil par son nom dans un tenant auquel il est autorisé en utilisant un certificat full access avec le rôle ROLE_GET_PROFILES$"
    )
    ...
    @When(
        "^un utilisateur avec le rôle ROLE_GET_PROFILES sans le bon niveau vérifie l'existence d'un profil par son nom dans un tenant auquel il est autorisé en utilisant un certificat full access avec le rôle ROLE_GET_PROFILES$"
    )
    ...
    @When(
        "^un utilisateur avec le rôle ROLE_GET_PROFILES vérifie l'existence d'un profil par son identifant d'un autre customer dans un tenant auquel il est autorisé en utilisant un certificat full access avec le rôle ROLE_GET_PROFILES$"
    )
