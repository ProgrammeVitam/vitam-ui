...
public class ApiIamExternalProfileCreationSteps extends CommonSteps {
    ...
    @When(
        "^un utilisateur avec le rôle ROLE_CREATE_PROFILES ajoute un nouveau profil dans un tenant auquel il est autorisé en utilisant un certificat full access avec le rôle ROLE_CREATE_PROFILES$"
    )
    public void un_utilisateur_avec_le_rôle_ROLE_CREATE_PROFILES_ajoute_un_nouveau_profil_dans_un_tenant_auquel_il_est_autorisé_en_utilisant_un_certificat_full_access_avec_le_rôle_ROLE_CREATE_PROFILES() {
        testContext.profileDto = getProfileRestClient(true, null, new String[] { ROLE_CREATE_PROFILES }).create(
    ...
    @When(
        "^un utilisateur avec le rôle ROLE_CREATE_PROFILES ajoute un profil en readonly dans un tenant auquel il est autorisé en utilisant un certificat full access avec le rôle ROLE_CREATE_PROFILES$"
    )
    public void un_utilisateur_avec_le_rôle_ROLE_CREATE_PROFILES_ajoute_un_profil_en_readonly_dans_un_tenant_auquel_il_est_autorisé_en_utilisant_un_certificat_full_access_avec_le_rôle_ROLE_CREATE_PROFILES() {
        ...
        try {
            getProfileRestClient(true, null, new String[] { ROLE_CREATE_PROFILES }).create(
                getSystemTenantUserAdminContext(),
                testContext.profileDto
            );
    ...
    public void le_serveur_refuse_la_création_du_profil_à_cause_du_readonly() {
        ...
        assertThat(testContext.exception.toString()).isEqualTo(
            "fr.gouv.vitamui.commons.api.exception.InvalidFormatException: Unable to create profile " +
            testContext.profileDto.getName() +
            ": readonly must be set to false"
        );
    }

    @When(
        "^un utilisateur avec le rôle ROLE_CREATE_PROFILES ajoute un profil mais avec un mauvais client dans un tenant auquel il est autorisé en utilisant un certificat full access avec le rôle ROLE_CREATE_PROFILES$"
    )
    ...
    public void le_serveur_refuse_la_création_du_profil_à_cause_du_mauvais_client() {
        assertThat(testContext.exception).isNotNull();
        assertThat(testContext.exception.toString()).isEqualTo(
            "fr.gouv.vitamui.commons.api.exception.InvalidFormatException: Unable to create profile: customerId " +
            CLIENT1_CUSTOMER_ID +
            " is not allowed"
        );
    }
    ...
    @When(
        "^un utilisateur avec le rôle ROLE_CREATE_PROFILES ajoute un profil mais avec un mauvais tenant dans un tenant auquel il est autorisé en utilisant un certificat full access avec le rôle ROLE_CREATE_PROFILES$"
    )
    ...
    public void le_serveur_refuse_la_création_du_profil_à_cause_du_mauvais_tenant() {
        assertThat(testContext.exception).isNotNull();
        assertThat(testContext.exception.toString()).isEqualTo(
            "fr.gouv.vitamui.commons.api.exception.InvalidFormatException: Unable to create profile: tenantIdentifier " +
            client1TenantIdentifier +
            " is not allowed"
        );
    }

    @When(
        "^un utilisateur avec le rôle ROLE_CREATE_PROFILES ajoute un profil avec un nom existant dans un tenant auquel il est autorisé en utilisant un certificat full access avec le rôle ROLE_CREATE_PROFILES$"
    )
    ...
    public void le_serveur_refuse_la_création_du_profil_à_cause_du_paramètre_manquant() {
        ...
        assertThat(testContext.exception.toString()).isEqualTo(
            "fr.gouv.vitamui.commons.api.exception.InvalidFormatException: Unable to create profile " +
            testContext.profileDto.getName() +
            ": externalParamId must be provided"
        );
    }

    @When(
        "^un utilisateur avec le rôle ROLE_CREATE_PROFILES ajoute un profil avec un rôle qu'il ne possède pas dans un tenant auquel il est autorisé en utilisant un certificat full access avec le rôle ROLE_CREATE_PROFILES$"
    )
    ...
    public void cet_utilisateur_crée_un_nouveau_profil_avec_pour_attribut_le_niveau_TEST_BIS(
        ...
        try {
            testContext.profileDto = getProfileRestClient()
                .create(getContext(testContext.mainTenant, testContext.tokenUser), profile);
        } catch (final RuntimeException e) {
            testContext.exception = e;
        }
    }
    ...
    public void cet_utilisateur_crée_un_nouveau_profil_utlisateur_avec_pour_attribut_le_niveau_XXX(
        final LevelParameterType level
    ) throws Exception {
        final ProfileDto profile = FactoryDto.buildDto(ProfileDto.class);
        profile.setLevel(level.getData());
    ...
    public void le_niveau_du_nouvel_profil_est_bien_le_niveau(final LevelParameterType level) throws Exception {
        assertThat(testContext.profileDto.getLevel())
            .overridingErrorMessage("Le niveau de l'utilisateur créé est null")
            .isNotNull();
        assertThat(testContext.profileDto.getLevel())
            .overridingErrorMessage("Le niveau de l'utilisateur créé n'est pas le niveau \"" + level.getData() + "\"")
