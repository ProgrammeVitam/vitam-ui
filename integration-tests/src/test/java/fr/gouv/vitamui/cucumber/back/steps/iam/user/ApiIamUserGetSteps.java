...
public class ApiIamUserGetSteps extends CommonSteps {
    ...
    @When(
        "^un utilisateur avec le rôle ROLE_GET_USERS récupère tous les utilisateurs dans un tenant auquel il est autorisé en utilisant un certificat full access avec le rôle ROLE_GET_USERS$"
    )
    ...
    @When(
        "^un utilisateur avec le rôle ROLE_GET_USERS récupère un utilisateur par son identifiant dans un tenant auquel il est autorisé en utilisant un certificat full access avec le rôle ROLE_GET_USERS$"
    )
    ...
    @When(
        "^un utilisateur avec le rôle ROLE_GET_USERS récupère un utilisateur d'un autre client par son identifiant dans un tenant auquel il est autorisé en utilisant un certificat full access avec le rôle ROLE_GET_USERS$"
    )
    public void un_utilisateur_avec_le_rôle_ROLE_GET_USERS_récupère_un_utilisateur_d_un_autre_client_par_son_identifiant_dans_un_tenant_auquel_il_est_autorisé_en_utilisant_un_certificat_full_access_avec_le_rôle_ROLE_GET_USERS() {
        try {
            userDto = getUserRestClient(
                true,
                new Integer[] { proofTenantIdentifier },
                new String[] { ROLE_GET_USERS }
            ).getOne(
                getContext(
                    proofTenantIdentifier,
                    tokenUserTest(new String[] { ROLE_GET_USERS }, proofTenantIdentifier, CLIENT1_CUSTOMER_ID, "")
                ),
                SYSTEM_USER_ID,
                Optional.empty()
            );
    ...
    @When(
        "^un utilisateur avec le rôle ROLE_GET_USERS sans le bon niveau récupère un utilisateur par son identifiant dans un tenant auquel il est autorisé en utilisant un certificat full access avec le rôle ROLE_GET_USERS$"
    )
    ...
    @When(
        "^un utilisateur avec le rôle ROLE_GET_USERS récupère tous les utilisateurs avec pagination dans un tenant auquel il est autorisé en utilisant un certificat full access avec le rôle ROLE_GET_USERS$"
    )
    ...
    @When(
        "^un utilisateur avec le rôle ROLE_GET_USERS récupère tous les utilisateurs d'un autre client avec pagination dans un tenant auquel il est autorisé en utilisant un certificat full access avec le rôle ROLE_GET_USERS$"
    )
    ...
    public void il_existe_plusieurs_utilisateurs_de_différents_niveaux(final List<List<String>> levels)
        throws Exception {
        ...
        for (final List<String> levelsList : levels) {
            for (final String level : levelsList) {
                ...
                tokenUser(
                    new String[] { ServicesData.ROLE_GET_USERS },
                    testContext.customerId,
                    "level" + convertedLevel + "@test.com",
                    convertedLevel,
                    testContext.mainTenant,
                    "idExistingUser" + convertedLevel
                );
    ...
    public void cet_utilisateur_récupère_les_utilisateurs_de_niveau_TEST(final LevelParameterType level)
        throws Exception {
        try {
    ...
    public void la_liste_renvoyée_par_le_serveur_ne_contient_pas_l_utilisateur_de_niveau_vide(
        final LevelParameterType level
    ) throws Exception {
        assertThat(
            paginatedUsers.stream().filter(user -> user.getId().equals(existingUsers.get(level.getData()))).findFirst()
        )
            .overridingErrorMessage(
                "La liste des utilisateurs récupérés contient l'utilisateur de niveau " + level.getData()
            )
            .isEmpty();
    }

    @Then("la liste renvoyée par le serveur contient l'utilisateur de niveau {level}")
    public void la_liste_renvoyée_par_le_serveur_contient_l_utilisateur_de_niveau_TEST(final LevelParameterType level)
        throws Exception {
        assertThat(
            paginatedUsers.stream().filter(user -> user.getId().equals(existingUsers.get(level.getData()))).findFirst()
        )
            .overridingErrorMessage(
                "La liste des utilisateurs récupérés ne contient pas l'utilisateur de niveau " + level.getData()
            )
            .isNotEmpty();
    }

    @Then("^la liste renvoyée par le serveur contient l'utilisateur ayant effectué l'action de récupération$")
    public void la_liste_renvoyée_par_le_serveur_contient_l_utilisateur_ayant_effectué_l_action_de_récupération()
        throws Exception {
        assertThat(paginatedUsers.stream().filter(user -> user.getId().equals(TestConstants.TESTS_USER_ID)).findFirst())
            .overridingErrorMessage(
                "La liste des utilisateurs récupérés ne contient pas l'utilisateur ayant effectué l'action getAllPaginated"
            )
            .isNotEmpty();
    }

    @Then("^la liste renvoyée par le serveur ne contient pas l'utilisateur ayant effectué l'action de récupération$")
    public void la_liste_renvoyée_par_le_serveur_ne_contient_pas_l_utilisateur_ayant_effectué_l_action_de_récupération()
        throws Exception {
        assertThat(paginatedUsers.stream().filter(user -> user.getId().equals(TestConstants.TESTS_USER_ID)).findFirst())
            .overridingErrorMessage(
                "La liste des utilisateurs récupérés contient l'utilisateur ayant effectué l'action getAllPaginated"
            )
            .isEmpty();
    }
    ...
    protected Collection<UserDto> getAllPaginatedCustom(final Optional<String> level) {
        ...
        return getUserRestClient()
            .getAllPaginated(
                getContext(testContext.mainTenant, testContext.tokenUser),
                0,
                Integer.MAX_VALUE,
                Optional.of(criteria.toJson()),
                Optional.empty(),
                Optional.empty(),
                Optional.empty()
            )
