...
public class ApiIamExternalSubrogationCreationSteps extends CommonSteps {
    ...
    @When(
        "^un utilisateur avec le rôle ROLE_CREATE_SUBROGATIONS ajoute une nouvelle subrogation dans un tenant auquel il est autorisé en utilisant un certificat full access avec le rôle ROLE_CREATE_SUBROGATIONS$"
    )
    ...
    @When(
        "^un utilisateur avec le rôle ROLE_CREATE_SUBROGATIONS ajoute une nouvelle subrogation pour un subrogé non subrogeable dans un tenant auquel il est autorisé en utilisant un certificat full access avec le rôle ROLE_CREATE_SUBROGATIONS$"
    )
    ...
    public void le_serveur_refuse_la_création_de_la_subrogation_à_cause_de_l_utilisateur() {
        ...
        assertThat(testContext.exception.toString()).isEqualTo(
            "fr.gouv.vitamui.commons.api.exception.InvalidFormatException:  User is not subrogeable"
        );
    }

    @When(
        "^un utilisateur avec le rôle ROLE_CREATE_SUBROGATIONS ajoute une nouvelle subrogation pour un subrogé dont le client est non subrogeable dans un tenant auquel il est autorisé en utilisant un certificat full access avec le rôle ROLE_CREATE_SUBROGATIONS$"
    )
    public void n_utilisateur_avec_le_rôle_ROLE_CREATE_SUBROGATIONS_ajoute_une_nouvelle_subrogation_pour_un_subrogé_dont_le_client_est_non_subrogeable_dans_un_tenant_auquel_il_est_autorisé_en_utilisant_un_certificat_full_access_avec_le_rôle_ROLE_CREATE_SUBROGATIONS() {
        try {
            ...
            final SubrogationDto subrogationDto = IamDtoBuilder.buildSubrogationDto(
                null,
    ...
    public void le_serveur_refuse_la_création_de_la_subrogation_à_cause_du_client() {
        assertThat(testContext.exception).isNotNull();
        assertThat(testContext.exception.toString()).isEqualTo(
            "fr.gouv.vitamui.commons.api.exception.InvalidFormatException:  Customer is not subrogeable"
        );
    }

    @When(
        "^un utilisateur avec le rôle ROLE_CREATE_SUBROGATIONS ajoute une nouvelle subrogation pour un subrogateur non existant dans un tenant auquel il est autorisé en utilisant un certificat full access avec le rôle ROLE_CREATE_SUBROGATIONS$"
    )
    ...
    @When(
        "^un utilisateur avec le rôle ROLE_CREATE_SUBROGATIONS ajoute une nouvelle subrogation pour un subrogé non existant dans un tenant auquel il est autorisé en utilisant un certificat full access avec le rôle ROLE_CREATE_SUBROGATIONS$"
    )
    ...
    @When(
        "^un utilisateur avec le rôle ROLE_CREATE_SUBROGATIONS ajoute une nouvelle subrogation pour un subrogé désactivé dans un tenant auquel il est autorisé en utilisant un certificat full access avec le rôle ROLE_CREATE_SUBROGATIONS$"
    )
    ...
    public void le_serveur_refuse_la_création_de_la_subrogation_à_cause_du_subrogé_désactivé() {
        assertThat(testContext.exception).isNotNull();
        assertThat(testContext.exception.toString()).isEqualTo(
            "fr.gouv.vitamui.commons.api.exception.InvalidFormatException: User status is not enabled"
        );
    }
}
