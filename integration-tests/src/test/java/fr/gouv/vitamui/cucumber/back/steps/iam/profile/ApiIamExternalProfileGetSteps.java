...
import fr.gouv.vitamui.iam.client.ProfileExternalRestClient;
...
public class ApiIamExternalProfileGetSteps extends CommonSteps {
    ...
    @When(
        "^un utilisateur avec le rôle ROLE_GET_PROFILES récupère tous les profils dans un tenant auquel il est autorisé en utilisant un certificat full access avec le rôle ROLE_GET_PROFILES$"
    )
    ...
    @When(
        "^un utilisateur avec le rôle ROLE_GET_PROFILES sans le bon niveau récupère tous les profils dans un tenant auquel il est autorisé en utilisant un certificat full access avec le rôle ROLE_GET_PROFILES$"
    )
    ...
    public void le_serveur_ne_retourne_que_le_profil_de_l_utilisateur() throws Exception {
        ...
        assertThat(profileDtos.get(0).getLevel())
            .overridingErrorMessage(
                "Le profil retourné n'a pas le niveau attendu : " +
                profileDtos.get(0).getLevel() +
                " au lieu de " +
                testContext.authUserDto.getLevel()
            )
    ...
    @When(
        "^un utilisateur avec le rôle ROLE_GET_PROFILES récupère un profil par son identifiant dans un tenant auquel il est autorisé en utilisant un certificat full access avec le rôle ROLE_GET_PROFILES$"
    )
    public void un_utilisateur_avec_le_rôle_ROLE_GET_PROFILES_récupère_un_profil_par_son_identifiant_dans_un_tenant_auquel_il_est_autorisé_en_utilisant_un_certificat_full_access_avec_le_rôle_ROLE_GET_PROFILES() {
        profileDto = getProfileRestClient()
            .getOne(getSystemTenantUserAdminContext(), SYSTEM_USER_PROFILE_ID, Optional.empty());
    }
    ...
    @When(
        "^un utilisateur avec le rôle ROLE_GET_PROFILES sans le bon niveau récupère un profil par son identifiant dans un tenant auquel il est autorisé en utilisant un certificat full access avec le rôle ROLE_GET_PROFILES$"
    )
    public void un_utilisateur_avec_le_rôle_ROLE_GET_PROFILES_sans_le_bon_niveau_récupère_un_profil_par_son_identifiant_dans_un_tenant_auquel_il_est_autorisé_en_utilisant_un_certificat_full_access_avec_le_rôle_ROLE_GET_PROFILES() {
        try {
            profileDto = getGoodClient()
                .getOne(getGoodContextWithWrongLevel(), SYSTEM_USER_PROFILE_ID, Optional.empty());
        } catch (final RuntimeException e) {
            testContext.exception = e;
        }
    }
    ...
    @When(
        "^un utilisateur avec le rôle ROLE_GET_PROFILES récupère tous les profils avec pagination dans un tenant auquel il est autorisé en utilisant un certificat full access avec le rôle ROLE_GET_PROFILES$"
    )
    public void un_utilisateur_avec_le_rôle_ROLE_GET_PROFILES_récupère_tous_les_profils_avec_pagination_dans_un_tenant_auquel_il_est_autorisé_en_utilisant_un_certificat_full_access_avec_le_rôle_ROLE_GET_PROFILES() {
        final PaginatedValuesDto<ProfileDto> profiles = getProfileRestClient()
            .getAllPaginated(
    ...
    @When(
        "^un utilisateur avec le rôle ROLE_GET_PROFILES sans le bon niveau récupère tous les profils avec pagination dans un tenant auquel il est autorisé en utilisant un certificat full access avec le rôle ROLE_GET_PROFILES$"
    )
    public void un_utilisateur_avec_le_rôle_ROLE_GET_PROFILES_sans_le_bon_niveau_récupère_tous_les_profils_avec_pagination_dans_un_tenant_auquel_il_est_autorisé_en_utilisant_un_certificat_full_access_avec_le_rôle_ROLE_GET_PROFILES() {
        final PaginatedValuesDto<ProfileDto> profiles = getGoodClient()
            .getAllPaginated(
                getGoodContextWithWrongLevel(),
                0,
                10,
                Optional.empty(),
                Optional.empty(),
