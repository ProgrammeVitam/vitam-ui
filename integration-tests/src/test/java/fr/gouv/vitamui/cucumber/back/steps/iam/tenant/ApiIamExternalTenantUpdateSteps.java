...
public class ApiIamExternalTenantUpdateSteps extends CommonSteps {
    ...
    public void un_tenant_a_été_créé() {
        ...
        testContext.savedTenantDto = getTenantRestClient()
            .create(getSystemTenantUserAdminContext(), testContext.tenantDto);
    }
    ...
    public void le_serveur_refuse_la_mise_à_jour_du_tenant() {
        ...
        assertThat(testContext.exception.toString()).contains(
            "Unable to update tenant " + testContext.savedTenantDto.getId()
        );
    }
    ...
    public void le_serveur_retourne_le_tenant_mis_à_jour() {
        assertThat(testContext.tenantDto.getName()).isEqualTo(
            TestConstants.UPDATED + testContext.savedTenantDto.getName()
        );
    }

    @When(
        "^un utilisateur avec le rôle ROLE_UPDATE_TENANTS met à jour un tenant dans un tenant auquel il est autorisé en utilisant un certificat full access avec le rôle ROLE_UPDATE_TENANTS$"
    )
    ...
    @When(
        "^un utilisateur avec le rôle ROLE_UPDATE_TENANTS met à jour un tenant en readonly dans un tenant auquel il est autorisé en utilisant un certificat full access avec le rôle ROLE_UPDATE_TENANTS$"
    )
    public void un_utilisateur_avec_le_rôle_ROLE_UPDATE_TENANTS_met_à_jour_un_tenant_en_readonly_dans_un_tenant_auquel_il_est_autorisé_en_utilisant_un_certificat_full_access_avec_le_rôle_ROLE_UPDATE_TENANTS() {
        ...
        try {
            testContext.tenantDto = getTenantRestClient()
                .update(getSystemTenantUserAdminContext(), testContext.tenantDto);
    ...
    @When(
        "^un utilisateur sans le rôle ROLE_UPDATE_TENANTS met à jour un tenant dans un tenant auquel il est autorisé en utilisant un certificat full access avec le rôle ROLE_UPDATE_TENANTS$"
    )
    public void un_utilisateur_sans_le_rôle_ROLE_UPDATE_TENANTS_met_à_jour_un_tenant_dans_un_tenant_auquel_il_est_autorisé_en_utilisant_un_certificat_full_access_avec_le_rôle_ROLE_UPDATE_TENANTS() {
        try {
            getTenantRestClient()
                .update(getContext(casTenantIdentifier, TestConstants.TOKEN_USER_CAS), buildTenantToUpdate());
        } catch (final RuntimeException e) {
            testContext.exception = e;
        }
    }
    ...
    @When(
        "^un utilisateur avec le rôle ROLE_UPDATE_TENANTS met à jour un tenant avec un nom existant dans un tenant auquel il est autorisé en utilisant un certificat full access avec le rôle ROLE_UPDATE_TENANTS$"
    )
    public void un_utilisateur_avec_le_rôle_ROLE_CREATE_TENANTS_met_à_jour_un_tenant_avec_un_nom_existant_dans_un_tenant_auquel_il_est_autorisé_en_utilisant_un_certificat_full_access_avec_le_rôle_ROLE_UPDATE_PROFILES() {
        ...
        try {
            testContext.tenantDto = getTenantRestClient()
                .update(getSystemTenantUserAdminContext(), testContext.savedTenantDto);
        } catch (final RuntimeException e) {
            testContext.exception = e;
        }
    }
