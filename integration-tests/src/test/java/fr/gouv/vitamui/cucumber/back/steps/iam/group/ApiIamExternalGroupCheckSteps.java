...
public class ApiIamExternalGroupCheckSteps extends CommonSteps {
    ...
    public void un_utilisateur_vérifie_l_existence_d_un_groupe_par_son_identifiant() {
        try {
            ...
            testContext.bResponse = getGroupRestClient()
    ...
    @When(
        "^un utilisateur avec le rôle ROLE_GET_GROUPS vérifie l'existence d'un groupe par son nom dans un tenant auquel il est autorisé en utilisant un certificat full access avec le rôle ROLE_GET_GROUPS$"
    )
    ...
    @When(
        "^un utilisateur avec le rôle ROLE_GET_GROUPS sans le bon niveau vérifie l'existence d'un groupe par son nom dans un tenant auquel il est autorisé en utilisant un certificat full access avec le rôle ROLE_GET_GROUPS$"
    )
    public void un_utilisateur_avec_le_rôle_ROLE_GET_GROUPS_sans_le_bon_niveau_vérifie_l_existence_d_un_groupe_par_son_nom_dans_un_tenant_auquel_il_est_autorisé_en_utilisant_un_certificat_full_access_avec_le_rôle_ROLE_GET_GROUPS() {
        ...
        testContext.bResponse = getGroupRestClient(
            ...
            new String[] { ROLE_GET_GROUPS }
        ).checkExist(
            getContext(
                proofTenantIdentifier,
                tokenUserTest(new String[] { ROLE_GET_GROUPS }, proofTenantIdentifier, SYSTEM_CUSTOMER_ID, "WRONGLEVEL")
            ),
            criteria.toJson()
        );
    }
