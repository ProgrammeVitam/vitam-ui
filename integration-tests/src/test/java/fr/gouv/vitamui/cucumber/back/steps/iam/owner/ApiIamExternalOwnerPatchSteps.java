...
public class ApiIamExternalOwnerPatchSteps extends CommonSteps {
    ...
    @When(
        "^un utilisateur avec le rôle ROLE_UPDATE_OWNERS met à jour partiellement un propriétaire dans un tenant auquel il est autorisé en utilisant un certificat full access avec le rôle ROLE_UPDATE_OWNERS$"
    )
    ...
    public void le_serveur_retourne_le_propriétaire_partiellement_mis_à_jour() {
        ...
        assertThat(testContext.ownerDto.getCompanyName()).isEqualTo(
            UPDATED + testContext.savedOwnerDto.getCompanyName()
        );
    }
    ...
    public void une_trace_de_mise_à_jour_du_propriétaire_est_présente_dans_vitam() throws InterruptedException {
        super.testTrace(
            ...
            testContext.ownerDto.getIdentifier(),
            "owners",
