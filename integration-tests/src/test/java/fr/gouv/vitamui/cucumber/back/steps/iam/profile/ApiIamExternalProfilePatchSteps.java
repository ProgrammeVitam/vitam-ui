...
public class ApiIamExternalProfilePatchSteps extends CommonSteps {
    ...
    @When(
        "^un utilisateur avec le rôle ROLE_UPDATE_PROFILES met à jour partiellement un profil dans un tenant auquel il est autorisé en utilisant un certificat full access avec le rôle ROLE_UPDATE_PROFILES$"
    )
    ...
    public void le_serveur_retourne_le_profil_partiellement_mis_à_jour() {
        ...
        assertThat(testContext.profileDto.getDescription()).isEqualTo(
            UPDATED + testContext.savedProfileDto.getDescription()
        );
    }

    @When(
        "^un utilisateur avec le rôle ROLE_UPDATE_PROFILES met à jour partiellement un profil en readonly dans un tenant auquel il est autorisé en utilisant un certificat full access avec le rôle ROLE_UPDATE_PROFILES$"
    )
    ...
    public void le_serveur_refuse_la_mise_à_jour_partielle_du_profil_à_cause_du_readonly() {
        ...
        assertThat(testContext.exception.toString()).isEqualTo(
            "fr.gouv.vitamui.commons.api.exception.InvalidFormatException: Unable to patch profile " +
            testContext.profileDto.getId() +
            ": readonly must be set to false"
        );
    }

    @When(
        "^un utilisateur avec le rôle ROLE_UPDATE_PROFILES met à jour partiellement un profil mais avec un mauvais client dans un tenant auquel il est autorisé en utilisant un certificat full access avec le rôle ROLE_UPDATE_PROFILES$"
    )
    ...
    public void le_serveur_refuse_la_mise_à_jour_partielle_du_profil_à_cause_du_mauvais_client() {
        assertThat(testContext.exception).isNotNull();
        assertThat(testContext.exception.toString()).isEqualTo(
            "fr.gouv.vitamui.commons.api.exception.InvalidFormatException: Unable to patch profile: customerId " +
            CLIENT1_CUSTOMER_ID +
            " is not allowed"
        );
    }
    ...
    @When(
        "^un utilisateur avec le rôle ROLE_UPDATE_PROFILES met à jour partiellement un profil mais avec un mauvais tenant dans un tenant auquel il est autorisé en utilisant un certificat full access avec le rôle ROLE_UPDATE_PROFILES$"
    )
    ...
    public void le_serveur_refuse_la_mise_à_jour_partielle_du_profil_à_cause_du_mauvais_tenant() {
        ...
        assertThat(testContext.exception.toString()).isEqualTo(
            "fr.gouv.vitamui.commons.api.exception.InvalidFormatException: Unable to patch profile " +
            testContext.profileDto.getId() +
            ": key tenantIdentifiant is not allowed"
        );
    }

    @When(
        "^un utilisateur avec le rôle ROLE_UPDATE_PROFILES met à jour partiellement un profil avec un nom existant dans un tenant auquel il est autorisé en utilisant un certificat full access avec le rôle ROLE_UPDATE_PROFILES$"
    )
    ...
    @When(
        "^un utilisateur avec le rôle ROLE_UPDATE_PROFILES sans le bon niveau met à jour partiellement un profil dans un tenant auquel il est autorisé en utilisant un certificat full access avec le rôle ROLE_UPDATE_PROFILES$"
    )
    public void un_utilisateur_avec_le_rôle_ROLE_UPDATE_PROFILES_sans_le_bon_niveau_met_à_jour_partiellement_un_profil_dans_un_tenant_auquel_il_est_autorisé_en_utilisant_un_certificat_full_access_avec_le_rôle_ROLE_UPDATE_PROFILES() {
        final Map<String, Object> dto = buildProfileToPatch();
        try {
            getProfileRestClient(true, null, new String[] { ROLE_UPDATE_PROFILES }).patch(
                getContext(
                    proofTenantIdentifier,
                    tokenUserTest(
                        new String[] { ROLE_UPDATE_PROFILES },
                        proofTenantIdentifier,
                        SYSTEM_CUSTOMER_ID,
                        "WRONGLEVEL"
                    )
    ...
    @When(
        "^un utilisateur avec le rôle ROLE_UPDATE_PROFILES désactive un profil dans un tenant auquel il est autorisé en utilisant un certificat full access avec le rôle ROLE_UPDATE_PROFILES$"
    )
    ...
    public void le_serveur_refuse_la_mise_à_jour_partielle_du_profil_à_cause_de_la_désactivation() {
        assertThat(testContext.exception).isNotNull();
        assertThat(
            testContext.exception
                .toString()
                .startsWith(
                    "fr.gouv.vitamui.commons.api.exception.InvalidFormatException: Unable to patch profile " +
                    testContext.profileDto.getId() +
                    ": the profile is referenced by"
                )
        ).isTrue();
    }

    @When(
        "^un utilisateur avec le rôle ROLE_UPDATE_PROFILES met à jour partiellement un profil avec un rôle qu'il ne possède pas dans un tenant auquel il est autorisé en utilisant un certificat full access avec le rôle ROLE_UPDATE_PROFILES$"
    )
    ...
    public void une_trace_de_mise_à_jour_du_profil_est_présente_dans_vitam() throws InterruptedException {
        super.testTrace(
            testContext.profileDto.getCustomerId(),
            testContext.profileDto.getIdentifier(),
            "profiles",
