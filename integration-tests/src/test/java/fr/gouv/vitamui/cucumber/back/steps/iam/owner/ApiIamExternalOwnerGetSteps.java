...
public class ApiIamExternalOwnerGetSteps extends CommonSteps {
    ...
    @When(
        "^un utilisateur avec le rôle ROLE_GET_OWNERS récupère un propriétaire par son identifiant dans un tenant auquel il est autorisé en utilisant un certificat full access avec le rôle ROLE_GET_OWNERS$"
    )
    public void un_utilisateur_avec_le_rôle_ROLE_GET_OWNERS_récupère_un_propriétaire_par_son_identifiant_dans_un_tenant_auquel_il_est_autorisé_en_utilisant_un_certificat_full_access_avec_le_rôle_ROLE_GET_OWNERS() {
        testContext.ownerDto = getOwnerRestClient()
            .getOne(getSystemTenantUserAdminContext(), TestConstants.SYSTEM_OWNER_ID, Optional.empty());
    }
