...
public class ApiIamExternalCustomerPatchSteps extends CommonSteps {
    ...
    @When(
        "^un utilisateur avec le rôle ROLE_UPDATE_CUSTOMERS met à jour partiellement un client dans un tenant auquel il est autorisé en utilisant un certificat full access avec le rôle ROLE_UPDATE_CUSTOMERS$"
    )
    public void un_utilisateur_avec_le_rôle_ROLE_UPDATE_CUSTOMERS_met_à_jour_partiellement_un_client_dans_un_tenant_auquel_il_est_autorisé_en_utilisant_un_certificat_full_access_avec_le_rôle_ROLE_UPDATE_CUSTOMERS() {
        ...
        try {
            testContext.basicCustomerDto = getCustomerWebClient()
                .patch(getSystemTenantUserAdminContext(), dto.get("id").toString(), patchFormData);
    ...
    public void le_serveur_retourne_le_client_partiellement_mis_à_jour() {
        ...
        assertThat(testContext.basicCustomerDto.getCompanyName()).isEqualTo(
            UPDATED + testContext.savedBasicCustomerDto.getCompanyName()
        );
    }
