...
public class ApiIamExternalGroupPatchSteps extends CommonSteps {
    ...
    @When(
        "^un utilisateur avec le rôle ROLE_UPDATE_GROUPS met à jour partiellement un groupe dans un tenant auquel il est autorisé en utilisant un certificat full access avec le rôle ROLE_UPDATE_GROUPS$"
    )
    ...
    public void le_serveur_retourne_le_groupe_partiellement_mis_à_jour() {
        ...
        assertThat(testContext.groupDto.getDescription()).isEqualTo(
            UPDATED + testContext.savedGroupDto.getDescription()
        );
    }

    @When(
        "^un utilisateur avec le rôle ROLE_UPDATE_GROUPS met à jour partiellement un groupe en readonly dans un tenant auquel il est autorisé en utilisant un certificat full access avec le rôle ROLE_UPDATE_GROUPS$"
    )
    ...
    public void le_serveur_refuse_la_mise_à_jour_partielle_du_groupe_à_cause_du_readonly() {
        ...
        assertThat(testContext.exception.toString()).isEqualTo(
            "fr.gouv.vitamui.commons.api.exception.InvalidFormatException: Unable to update group " +
            testContext.groupDto.getId() +
            ": readonly must be set to false"
        );
    }

    @When(
        "^un utilisateur avec le rôle ROLE_UPDATE_GROUPS mais avec un mauvais client met à jour partiellement un groupe dans un tenant auquel il est autorisé en utilisant un certificat full access avec le rôle ROLE_UPDATE_GROUPS$"
    )
    public void un_utilisateur_avec_le_rôle_ROLE_UPDATE_GROUPS_mais_avec_un_mauvais_client_met_à_jour_partiellement_un_groupe_dans_un_tenant_auquel_il_est_autorisé_en_utilisant_un_certificat_full_access_avec_le_rôle_ROLE_UPDATE_GROUPS() {
        ...
        testContext.level = "";
        try {
            getGroupRestClient(true, null, new String[] { ROLE_UPDATE_GROUPS }).patch(
                getContext(
                    client1TenantIdentifier,
                    tokenUserTest(
                        new String[] { ROLE_UPDATE_GROUPS },
                        client1TenantIdentifier,
                        CLIENT1_CUSTOMER_ID,
                        testContext.level
                    )
    ...
    @When(
        "^un utilisateur avec le rôle ROLE_UPDATE_GROUPS met à jour partiellement un groupe avec un nom existant dans un tenant auquel il est autorisé en utilisant un certificat full access avec le rôle ROLE_UPDATE_GROUPS$"
    )
    ...
    @When(
        "^un utilisateur avec le rôle ROLE_UPDATE_GROUPS sans le bon niveau met à jour partiellement un groupe dans un tenant auquel il est autorisé en utilisant un certificat full access avec le rôle ROLE_UPDATE_GROUPS$"
    )
    ...
    @When(
        "^un utilisateur avec le rôle ROLE_UPDATE_GROUPS met à jour partiellement un groupe avec un profil inexistant dans un tenant auquel il est autorisé en utilisant un certificat full access avec le rôle ROLE_UPDATE_GROUPS$"
    )
    ...
    @When(
        "^un utilisateur avec le rôle ROLE_UPDATE_GROUPS met à jour partiellement un groupe avec un profil d'un autre client dans un tenant auquel il est autorisé en utilisant un certificat full access avec le rôle ROLE_UPDATE_GROUPS$"
    )
    ...
    public void une_trace_de_mise_à_jour_du_groupe_est_présente_dans_vitam() throws InterruptedException {
        super.testTrace(
            testContext.groupDto.getCustomerId(),
            testContext.groupDto.getIdentifier(),
            "groups",
