...
public class ApiIamExternalTenantCreationSteps extends CommonSteps {
    ...
    @When(
        "^un utilisateur avec le rôle ROLE_CREATE_TENANTS ajoute un nouveau tenant dans un tenant auquel il est autorisé en utilisant un certificat full access avec le rôle ROLE_CREATE_TENANTS$"
    )
    ...
    @When(
        "^un utilisateur avec le rôle ROLE_CREATE_TENANTS ajoute un tenant avec un nom existant dans un tenant auquel il est autorisé en utilisant un certificat full access avec le rôle ROLE_CREATE_TENANTS$"
    )
    public void un_utilisateur_avec_le_rôle_ROLE_ROLE_CREATE_TENANTS_ajoute_un_tenant_avec_un_nom_existant_dans_un_tenant_auquel_il_est_autorisé_en_utilisant_un_certificat_full_access_avec_le_rôle_ROLE_CREATE_TENANTS() {
        ...
        try {
            ...
            final TenantDto tenantDto = IamDtoBuilder.buildTenantDto(
                null,
                testContext.tenantDto.getName(),
                null,
                rownerDto.getId(),
                SYSTEM_CUSTOMER_ID
            );
    ...
    public void le_serveur_refuse_la_création_du_tenant_à_cause_du_nom_existant() {
        ...
        assertThat(testContext.exception.toString()).isEqualTo(
            "fr.gouv.vitamui.commons.api.exception.InvalidFormatException: Unable to create tenant " +
            testContext.tenantDto.getName() +
    ...
    public void cet_utilisateur_ajoute_un_nouveau_tenant() {
        try {
            ...
            ).create(getContext(testContext.tenantIHMContext, testContext.tokenUser), createOwnerAndBuildTenant());
        } catch (final RuntimeException e) {
            testContext.exception = e;
        }
    }
