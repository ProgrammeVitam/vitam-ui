...
public class ApiIamExternalSubrogationAcceptSteps extends CommonSteps {
    ...
    public void un_utilisateur_accepte_la_subrogation() {
        subrogationDto = getSubrogationRestClient()
            .accept(
                getContext(testContext.tenantDto.getIdentifier(), testContext.authUserDto.getAuthToken()),
    ...
    public void un_autre_utilisateur_accepte_la_subrogation() {
        try {
            getSubrogationRestClient()
                .accept(getSystemTenantUserAdminContext(), testContext.savedSubrogationDto.getId());
    ...
    public void le_serveur_refuse_d_accepter_la_subrogation() {
        ...
        assertThat(testContext.exception.toString()).isEqualTo(
            "fr.gouv.vitamui.commons.api.exception.InvalidFormatException: Users " +
            TestConstants.SYSTEM_USER_PREFIX_EMAIL +
            CommonConstants.EMAIL_SEPARATOR +
            defaultEmailDomain +
            " can't accept subrogation of " +
