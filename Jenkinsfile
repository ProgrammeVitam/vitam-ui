...
//                '''
        ...
        stage('Checkmarx SCA step') {
           ...
           steps {
                ...
                sh './ScaResolver -s ui -n vitam-ui-${env.GIT_BRANCH} -u ${env.SERVICE_CX_SCA_USER} -p ${env.SERVICE_CX_SCA_PASSWORD} -a ${env.SERVICE_CX_SCA_ACCOUNT}' --server-url ${env.SERVICE_CX_SCA_SERVER} --authentication-server-url ${env.SERVICE_CX_SCA_AUTH_SERVER} -s ui' 
           }
        }
