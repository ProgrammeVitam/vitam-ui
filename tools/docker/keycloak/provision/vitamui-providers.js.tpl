...
const oidcProvider = {
  ...
  // Without this, CustomDelegatedAuthenticationClientLogoutAction returns no client and
  // the VitamUI logout leaves the Keycloak session open: the next login goes straight
  // back through without ever asking for credentials.
  propagateLogout: true,
...
const samlProvider = {
  ...
  mailAttribute: "",
  autoProvisioningEnabled: false,
  // Propagated through the SAML SLO endpoint carried by the IdP metadata.
  propagateLogout: true,
...
function upsertUser(id, userInfoId, email, lastname, sequenceId) {
  ...
  const user = {
    ...
    // NOMINATIVE is the only type that may authenticate: LoginPwdAuthenticationHandler
    // accepts `status == ENABLED && type == NOMINATIVE`, and GENERIC is reserved for the
    // generic accounts a nominative user subrogates (SubrogationService lists them by type).
    type: "NOMINATIVE",
    status: "ENABLED",
    otp: false,
    subrogeable: true,
