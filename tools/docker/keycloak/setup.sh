...
#   2. realigns both clients on the CAS URL declared in .env,
...
check_consistency() {
    ...
    stored=$(mongosh --quiet "$MONGO_URI" --eval \
        "print((db.getSiblingDB('${MONGO_DB}').providers.findOne({_id:'keycloak_test_idp_saml'})||{}).idpMetadata||'')" |
        grep -o '<ds:X509Certificate>[^<]*' | head -1 | cut -d'>' -f2) || true
    if [[ -z $stored ]]; then
    ...
    discovery=$(mongosh --quiet "$MONGO_URI" --eval \
        "print((db.getSiblingDB('${MONGO_DB}').providers.findOne({_id:'keycloak_test_idp_oidc'})||{}).discoveryUrl||'')")
...
# The imported realm carries the default CAS URL. Rather than templating the
# realm file, the two clients are patched through the admin API so that .env
...
patch_client() {
    local uuid="$1"
    shift
    local url="${KEYCLOAK_BASE_URL}/admin/realms/${KEYCLOAK_REALM}/clients/${uuid}"
...
patch_client "$OIDC_CLIENT_UUID" \
    ...
    '.clientId = $clientId | .secret = $secret | .redirectUris = [$redirect]'
