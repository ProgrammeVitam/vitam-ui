...
app_mode() {
  local app="$1" port="${NG_PORTS[$1]}" dist_dir index built
  ...
  dist_dir="$(env_value VITAMUI_DIST_DIR ../../../ui/ui-frontend/dist)"
  index=""
  [[ -f "$dist_dir/$app/index.html" ]] && index="$dist_dir/$app/index.html"
  [[ -f "$dist_dir/$app/browser/index.html" ]] && index="$dist_dir/$app/browser/index.html"
  if [[ -n "$index" ]]; then
    # Build date = mtime of the served index.html (same instant nginx exposes as Last-Modified).
    built="$(date -r "$index" '+%Y-%m-%d %H:%M' 2>/dev/null)"
    printf 'dist%s' "${built:+  (built $built)}"
  else
